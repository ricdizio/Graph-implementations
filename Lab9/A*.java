...
public class A*{
	...
	private Vertice openSet;
	private  closedSet;
	private Vertice goal[];

	public A*(GrafoNoDirigido G, String s)
	{
		...
		for (Vertice v: listadeVertices)
		{
			...
			if ( !s.equals(v.getId())) {
				goal[i] = v;
				this.costosHeuristica[doubleToInt(ver)] = 0.0;				
			}
			i = i + 1;
		}
		...
		while (openSet.size() > 0)
		{
			...
			for (Vertice v1: x.getListaDeAdyacencias()) 
			{
				...
				if (this.costos[doubleToInt(v1)] > (this.costos[doubleToInt(x)] + this.costo(x, v1))) 
				{
					...
					this.costosHeuristica[doubleToInt(v1)] = this.costos[doubleToInt(v1)]+this.costo(v1,goal[DoubleToInt(v1.getPeso())]);
